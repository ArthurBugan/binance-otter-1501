...
const addTotalSums = (orders: number[][]): number[][] => {
  ...
  return orders.map((order: number[], idx) => {
    ...
    if (typeof order[2] !== 'undefined') {
      ...
      updatedLevel[2] = parseFloat(totalSum.toString());
...
const addDepths = (orders: number[][], maxTotal: number): number[][] => {
  return orders.map(order => {
    if (typeof order[3] !== 'undefined') {
      ...
      const total = order[2].toString();

      const calculatedTotal: number = parseFloat(total);
...
const getMaxTotalSum = (orders: number[][]): number => {
  const totalSums: number[] = orders.map(order => parseFloat(order[2].toString()));
...
export const orderbook = createSlice({
  ...
  initialState,
  reducers: {
    addCoinValue: (state, { payload }) => {
      state.coinValue = parseFloat(payload)
    },
...
export const { addBids, addAsks, addExistingState, setGrouping, clearOrdersState, addCoinValue } = orderbook.actions;
...
export const selectCoinValue = (state: RootState): number => state.orderbook.coinValue;
